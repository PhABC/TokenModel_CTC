...
S.onset  = 100;  % onset of trial in ms S.aftcmt = 100; % Stop simulation after X ms S.dt = 1; % Time step in ms S.tau = 0.005; % Time constant
...
S.Fsteep(2) = 0.1  ;  % Steepness of activation function for population 2
...
%% Non-parallel computing    
    ...
    for net = 1:S.nbNet
         ...
         %Stimuli and Bias
         [Uall,stimTrial,idxStim(:,net),S] = ExtInputs(S,Stim);
         
         %
         
         %Simulation
         fprintf('Simulating %d trials for Network %d of %d ...\n',S.nbEx,net,S.nbNet)  	
         [FR{net},commit(:,net)] = trialLoop(S,net,hnorm,W,Uall,stimTrial,Stim,idxStim(:,net),Gtime);
